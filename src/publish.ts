...
async function main() {
    ...
    for (const adapter of ADAPTERS) {
        ...
        const envVarName = `ENABLE_${adapter.name
            .toUpperCase()
            .replace(/[^A-Z0-9]/g, "_")}`;
        ...
        if (bloggerAdapter && !dryRun) {
            ...
            if (!force && currentState) {
                ...
                if (
                    ...
                    try {
                        const result = await bloggerAdapter.update(
                            post,
                            currentState.postId
                        );
                        if (result.success) {
                    ...
                    logger.info(
                        `Post ${post.slug} already up-to-date on Blogger, using existing URL`
                    );
                    post.publishedUrl = currentState.url;
                }
            } else {
        ...
        const publishPromises = otherAdapters.map(async (adapter) => {
            ...
            if (!force && currentState) {
                ...
                if (
                    ...
                    logger.info(
                        `Updating post ${post.slug} on ${adapter.name}...`
                    );
                    try {
                    ...
                    logger.info(
                        `Skipping ${post.slug} on ${adapter.name} (already up-to-date)`
                    );
                    return {
                        platform: adapter.name,
                        success: true,
